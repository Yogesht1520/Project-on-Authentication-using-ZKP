...
if __name__ == "__main__":
    ...
    stored_password_hash = sha256_hash(stored_password)
    ...
    if entered_password_hash == stored_password_hash:
        print("Password is correct. Proceeding with Zero-Knowledge Proof...\n")
        secret = password

        zk_proof = ZeroKnowledgeProof(secret)

        commitment = zk_proof.create_commitment()
        print(f"{username}'s commitment: {commitment}")
